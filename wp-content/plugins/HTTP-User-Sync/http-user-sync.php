...
function user_sync() {
    ...
    foreach ($object->users as $user) {
        ...
        if (username_exists($username) && $memberStatus != 'delete') {
            // get the userid of that username to manipulate it
            $user = get_user_by('login', $username);
            $user_id = $user->ID;

            // if ($memberType == "realtor"){
            //   $result = CTXPS_Queries::add_membership_with_expiration($user_id, 26);
            // }
            ...
            update_user_meta($user_id, 'member_type', $memberType);
            ...
            update_user_meta($user_id, 'member_status', $memberStatus);

            $membersStatus[] = array('Member' => $username, 'Status' => 'Updated');
            $updatedNumber++;
        }
        ...
        if (username_exists($username) && $memberStatus == 'delete') {
            ...
            $membersStatus[] = array('Member' => $username, 'Status' => 'Deleted');
            $deletedNumber++;
            ...
            delete_user_meta($user_id, 'member_type', $memberType);
            ...
            delete_user_meta($user_id, 'member_status', $memberStatus);
        }
        ...
        if (!username_exists($username) && $memberStatus != 'delete') {
            ...
            $membersStatus[] = array('Member' => $username, 'Status' => 'Inserted');
            $insertedNumber++;
        }
    ...
    // Start fresh for each sync
    $errorLog = array();


    // Store everything we need to add to the log in an array
    $errorData = array(
        'user_info'  => array(
            'user_id'    => $user_id,
            'username'   => $username,
...
// IMPORTANT: server side validation return response with status
    ...
    if( !is_wp_error($user_id) ) {
        ...
        // Add the error message to the user data
        $errorData['error_message'] = $user_id->get_error_message();
        // Push our data to the error log array
        array_push( $errorLog, $errorData );
        // Set/update option with new error data
        update_option('error_report', $errorLog);
