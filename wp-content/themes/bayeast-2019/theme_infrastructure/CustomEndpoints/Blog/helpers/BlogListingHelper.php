...
class BlogListingHelper
{
        ...
        $post_text = $blogQuery->found_posts > 0 ? ' posts' : ' post';
          ...
          $blogs_html .= '<ul class="post-list">';
            if($blogQuery->have_posts()){
                ...
                $blogs_html .= '<div class="no-results"><h2>No Blogs found.</h2></div>';
            }
        $blogs_html .= '</ul>';
    ...
    private function getFilterHTML(){
      $catOptions = $this->getUsedCustomTaxonomyValues('category');

      $filter_html = '';
      $filter_html .= '<div class="post-tags container">';
        $filter_html .= '<div class="search">';
        $filter_html .= '<input type="text" name="search" id="select-filter-search" value="" placeholder="Search All News">';
        $filter_html .= '</div>';
    ...
    private function getBlogEntryHTML($post_id){
      ...
      $excerpt = mb_strimwidth(get_the_excerpt($post_id), 0, 250, '...');

      $blog_html = "";
      $blog_html .= '<li class="post">';
          $blog_html .= '<h2 class="post_title"><a href="' . $permalink . '">' . $post_title . '</a></h2>';
          $blog_html .= '<span class="post_cat-name">' . $terms . '</span>';
          $blog_html .= '<div class="post_excerpt">';
            $blog_html .= '<p>' . $excerpt . ' <a href="' . $permalink . '" class="excerpt-read-more">Read More »</a></p>';
          $blog_html .= '</div>';
      $blog_html .= '</li>';
